...
mod.setting(
    ...
    type=str,
    default='C:\\Users\\The Pterra\\AppData\\Roaming\\talon\\replay_drafts',
...
class _RecordingReplayer(object):
    ...
    def play_file(self, recording: pathlib.Path):
        ...
        print(subprocess.run([r"C:\Program Files\VideoLAN\VLC\vlc.exe", "--play-and-exit", "--intf", "dummy", "--no-interact", recording]))
        print('ok')
...
class Actions:
    ...
    def replay_saved_file_by_file_name(file_name: str):
        """Copy the file name of replay file"""
        global rr
        print(file_name + "***************")
        if os.path.exists(pathlib.Path(rr.saved_recording_directory, pathlib.Path(file_name))):
            app.notify("file  found" + file_name)
            rr.play_file(pathlib.Path(rr.saved_recording_directory, pathlib.Path(file_name)))
            clip.set_text(file_name)
        else:
            app.notify("file not found" + file_name)
